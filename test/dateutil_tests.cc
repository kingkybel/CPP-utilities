...
struct DR
{
    DR(string param, string const result, size_t line)
...
TEST_F(DateUtilTest, util_date_european_test)
{
    ...
    {
        ASSERT_EQ(string("not-a-date-time"), toString(scanDate(scanResult.param_)));
    }
    addDateFormat("%H:%Y");
...
TEST_F(DateUtilTest, util_date_american_test)
{
    ...
    for (auto &scanResult: scanResults)
    {
        ASSERT_TRUE(scanResult.correctResult());
    }
}
