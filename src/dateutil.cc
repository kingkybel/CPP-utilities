 ...
 * 3/8/1989 -> 03/08/1989
 */
string addLeadingZeros(string const &str)
{
    string      reval = "";
 ...
 */
time_t pt_to_time_t(ptime const &pt)
{
    ptime         timer_start(date(1'970, 1, 1));
    time_duration diff = pt - timer_start;
 ...
 */
inline bool isTimeOnly(string const &s)
{
    ...
    for (auto &format: formats())
    {
        ...
        if (reval != nullTime)
        {
            if (isTimeOnly(s))
            {
                ...
                reval       = ptime(
                    date(now.tm_year + 1'900, now.tm_mon + 1, now.tm_mday), // @suppress("Avoid magic numbers")
...
time_t seconds_from_epoch(string const &s)
{
 ...
 */
string setABYFlag(string const &fmt, char flag_letter)
{
    string flag = "%";
    flag += flag_letter;
    string const changeableFlags = "%a%A%b%B%y%Y";
    string       reval           = fmt;
...
void addDateFormat(string const &fmt)
{
...
vector<locale> &initDateFormats(DateFormatPreference pref)
{
    ...
    for (auto fmt: disambiguatedFormats)
    {
        ...
        for (auto const &y: {'Y', 'y'}) // try first all where the year has 4 digits
        {
            fmt = setABYFlag(fmt, y);
            for (auto const &a: {'A', 'a'}) // First "Monday", then "Mon"
            {
                fmt = setABYFlag(fmt, a);
                for (auto const b: {'B', 'b'}) // First "February" then "Feb"
                {
...
void imbueDateFormat(ostream &os, string const &fmt)
{
...
string dateString(ptime const &dt, string const &fmt)
{
    stringstream ss;
...
string timestamp(string const &fmt)
{
