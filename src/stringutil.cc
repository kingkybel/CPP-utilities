...
{
    static size_t maxIntLen  = asString(numeric_limits<VAR_INT>::max()).size();
    static size_t maxUintLen = asString(numeric_limits<VAR_UINT>::max()).size();
...
{
    const static map<ci_string, bool> VALID_BOOL = {{"true", true},
                                                    {"t", true},
                                                    {"yes", true},
                                                    {"y", true},
                                                    {"1", true},
...
{
    stripT(v, trimChars, static_cast<StripTrimMode>(((m & StripTrimMode::FRONT) | (m & StripTrimMode::BACK))));
}
