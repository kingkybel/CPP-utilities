...
// Re-create Solaris double_to_decimal, decimal_to_double, ...
...
namespace util
{
    ...
    ostream& operator<<(ostream& os, const decimal_record& dr)
    {
        os << "decimal_record { ds=" << dr.ds
                ...
                << ", fpclass=" << (dr.fpclass == fp_zero ? "fp_zero" :
    ...
    static const mpf_class minusOne(-1.0, defaultNumberOfBits);
    ...
    mpf_class round(mpf_class val, decimal_mode* pm)
    {
        ...
        if (pm != 0 && val != zero)
        {
            ...
            if (abs_val < one) // the value is between -1.0 and 1.0
            {
                ...
                stringstream ss;
                ss << fixed << setprecision(40) << abs_val;
                string s = ss.str();
            ...
            int shiftDpl = ((pm->df == floating_form) ?
                            pm->ndigits - exponent :
                            pm->ndigits);
                              ...
                              fp_exception_field_type *ps)
    {
        ...
        if (pd != 0 && px != 0)
        {
            ...
            if (*px == numeric_limits<quadruple>::infinity())
            {
                pd->fpclass = fp_infinity;
                return;
            }
            else if (*px == -numeric_limits<quadruple>::infinity())
            {
                pd->fpclass = fp_infinity;
                pd->sign = 1;
                return;
            }
            else if (*px == numeric_limits<quadruple>::quiet_NaN())
            {
                pd->fpclass = fp_quiet;
                return;
            }
            else if (*px == numeric_limits<quadruple>::signaling_NaN())
            {
            ...
            else
            {
                ...
                valStr = valStr.substr(0, pos) +
                        (valStr[valStr.size() - 1] != '.' ? valStr.substr(pos + 1) : "");
