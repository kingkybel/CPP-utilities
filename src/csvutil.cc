...
namespace util
{
int const CSVAnalyzer::xalloc_index = ios_base::xalloc();

std::string const CSV_COLUMN_TYPE_BOOL        = "bool";
std::string const CSV_COLUMN_TYPE_CHAR        = "char";
std::string const CSV_COLUMN_TYPE_INT         = "int";
std::string const CSV_COLUMN_TYPE_UINT        = "uint";
std::string const CSV_COLUMN_TYPE_FLOAT       = "float";
std::string const CSV_COLUMN_TYPE_STRING      = "string";
std::string const CSV_COLUMN_TYPE_DATE        = "date";
std::string const CSV_COLUMN_TYPE_GAUSSIAN    = "gaussian";
std::string const CSV_COLUMN_TYPE_EXPONENTIAL = "exponential";
...
CSVAnalyzer::CSVAnalyzer(string const &headerStr, string const &typeStr, string outSeparator)
...
bool CSVAnalyzer::splitLine(string const &str, vector<string> &result, string const &inSeparator)
{
...
bool CSVAnalyzer::setHeaders(string const &headerString, bool replaceHeaders, string const &inSeparator)
{
...
bool CSVAnalyzer::setTypes(string const &typeString, string const &inSeparator)
{
    vector<string> types;
...
string CSVAnalyzer::guessType(string const &stringVal)
{
...
bool CSVAnalyzer::createDefaultHeader(vector<string> const &values)
{
    string header;
...
bool CSVAnalyzer::createTypesFromValues(vector<string> const &values)
{
    string types;
...
bool CSVAnalyzer::setValues(string const &valueString, bool preserveRows, string const &inSeparator)
{
    ...
    for (size_t i = 0; i < values.size(); i++)
    {
        ...
        {
            data_[i].emplace_back(scanAs<VAR_BOOL>(values[i]));
        }
        else if (tp == CSV_COLUMN_TYPE_CHAR)
        {
            data_[i].emplace_back(scanAs<VAR_CHAR>(values[i]));
        }
        else if (tp == CSV_COLUMN_TYPE_INT)
        {
            data_[i].emplace_back(scanAs<VAR_INT>(values[i]));
        }
        else if (tp == CSV_COLUMN_TYPE_UINT)
        {
            data_[i].emplace_back(scanAs<VAR_UINT>(values[i]));
        }
        else if (tp == CSV_COLUMN_TYPE_FLOAT || tp == CSV_COLUMN_TYPE_GAUSSIAN || tp == CSV_COLUMN_TYPE_EXPONENTIAL)
        {
            data_[i].emplace_back(scanAs<VAR_FLOAT>(values[i]));
        }
        else if (tp == CSV_COLUMN_TYPE_STRING)
        {
            data_[i].emplace_back(scanAs<VAR_STRING>(values[i]));
        }
        else if (tp == CSV_COLUMN_TYPE_DATE)
        {
            data_[i].emplace_back(scanAs<VAR_DATE>(values[i]));
        }
    }
...
void CSVAnalyzer::appendColumn(string const &header, string const &tp, Var const &defaultValue)
{
    COLUMN_TYPE col;
...
Var CSVAnalyzer::getVar(string const &header, size_t index) const
{
...
vector<VAR_FLOAT> CSVAnalyzer::getFloatVector(std::string const &header) const
{
    auto found = headerIndex_.find(header);
...
CSVAnalyzer::COLUMN_TYPE_ITER CSVAnalyzer::begin(string const &header)
{
    auto found = headerIndex_.find(header);
...
CSVAnalyzer::COLUMN_TYPE_ITER CSVAnalyzer::end(string const &header)
{
    auto found = headerIndex_.find(header);
...
CSVAnalyzer CSVAnalyzer::getSub(vector<string> const &headers) const
{
    CSVAnalyzer reval;
    size_t      index = 0;
...
CSVAnalyzer CSVAnalyzer::getSub(vector<size_t> const &columns) const
{
    CSVAnalyzer reval;
    size_t      index = 0;
...
bool CSVAnalyzer::write(string const &filename, string const &outDelimiter, fileFormatType tp)
{
    bool     reval = false;
...
bool CSVAnalyzer::read(string const &filename, string const &inDelimiter, fileFormatType tp)
{
    bool reval = false;
...
CSVAnalyzer &CSVAnalyzer::operator<<(string const &row)
{
...
void CSVAnalyzer::convert(size_t col, size_t row, string const &tp)
{
...
bool CSVAnalyzer::eraseColumn(string const &header)
{
    auto found = headerIndex_.find(header);
...
ostream &operator<<(ostream &os, CSVAnalyzer const &csv)
{
