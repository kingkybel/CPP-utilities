...
#ifndef NS_UTIL_CSVUTIL_H
#define	NS_UTIL_CSVUTIL_H
...
namespace util
{
    ...
    struct index_error : public std::logic_error
    {
        ...
        : std::logic_error(std::string(tp == col ? "Column" : "Row") + " index " +
    ...
    /// Read/write comma-separated files and hold in an in-homogenous table
    /// with typed columns.
    /// Types can be
    /// <table>
    ...
    public:
        ...
        bool read(const std::string& filename = "",
                  ...
                  fileFormatType = defaultFileFormat);
        /// Write to file-system.
        bool write(const std::string& filename = "",
                   const std::string& outDelimiter = ",",
                   fileFormatType tp = defaultFileFormat);
        ...
        T_ get(size_t column, size_t line) const
        {
            ...
            if (line >= lines())
            {
                throw index_error(index_error::row, line, lines() - 1);
            }
