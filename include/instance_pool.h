...
namespace util
{
    ...
    struct InstancePool
    {
        typedef std::shared_ptr<T_> ObjectPointer;
        typedef std::unordered_set<ObjectPointer> ObjectContainerType;
        typedef typename ObjectContainerType::iterator ObjectContainerIterator;
    ...
    protected:
        ...
        static void addInstance(ObjectPointer pObject)
        {
            if (avail_.find(pObject) == avail_.end())
            {
                ...
                {
                    std::stringstream ss;
                    ss << "Instance pool has reached the maximum number "
                            << maxInstances
                            << " of instances in class '"
                            << typeid (T_).name()
                            << "'";
