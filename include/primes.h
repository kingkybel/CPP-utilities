...
#define DEBUG (false)
#define SHOW(v) { DEBUG && std::cout << #v << "=" << v << std::endl; }
    ...
    {
    private:
        const long double MAX_NUMBER =
                (const long double) std::numeric_limits<unsigned long long>::max();
        ...
        bool makePartialSieve()
        {
            ...
            for (size_t p = 2; p < ROOT_SIZE_ULL;)
            {
                ...
                while (p < ROOT_SIZE_ULL && sieve[p] == false)
                    p++;
                if ((long double) p * (long double) partialSieveSize < (long double) ROOT_SIZE_ULL)
                    partialSieveSize *= p;
            }
            return true;
        }
