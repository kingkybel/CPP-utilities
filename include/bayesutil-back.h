...
class Node : public NodeBase
{
     ...
     * incoming vertex_properties_t.
     *
     * @param incoming the set of incoming vertex_properties_t
    ...
    /**
     * @brief satisfy the hash-value requirement of graph-vertex_properties_t.
     *
...
/**
 * @brief Directed edge-type between vertex_properties_t in the Bayes-net.
 */
class Dependency
{
    public:
 ...
 * A uni-directed graph where vertex_properties_t have distributions attached.
 */
class BayesNet
{
    public:
    using DAG_TYPE        = abstract_base_graph<Node, Dependency>;
    using VERTEX_RESULT   = DAG_TYPE::vertex_result_t;
    using NODE_SET        = DAG_TYPE::node_set;
    using NODE_PTR_VECTOR = DAG_TYPE::node_ptrs;
    ...
    /**
     * @brief Add a cause/effect connection between two vertex_properties_t. If the vertex_properties_t do not yet
    ...
    /**
     * @brief Retrieve the set of vertex_properties_t connected to a node (in and out edge_properties_t)
     *
     * @param checkNode the node to check
     * @return node_set a set of all connected vertex_properties_t
     */
    NODE_SET connectedNodes(const Node &checkNode);

    /**
     * @brief Retrieve the set of all connected to a node (out edge_properties_t) constant version
     *
     * @param checkNode the node to check
     * @return node_set a set of all connected vertex_properties_t
     */
    ...
    /**
     * @brief Retrieve the set of children vertex_properties_t connected to a node (out edge_properties_t)
     *
     * @param checkNode the node to check
     * @return node_set a set of all children vertex_properties_t
     */
    NODE_SET childrenNodes(const Node &checkNode);

    /**
     * @brief Retrieve the set of children vertex_properties_t connected to a node (out edge_properties_t) constant version
     *
     * @param checkNode the node to check
     * @return node_set a set of all children vertex_properties_t
     */
    ...
    /**
     * @brief Retrieve the set of parent vertex_properties_t connected to a node (in edge_properties_t)
     *
     * @param checkNode the node to check
     * @return node_set a set of all parent vertex_properties_t
     */
    NODE_SET parentNodes(const Node &checkNode);

    /**
     * @brief Retrieve the set of parent vertex_properties_t connected to a node (in edge_properties_t) constant version
     *
     * @param checkNode the node to check
     * @return node_set a set of all parent vertex_properties_t
     */
    ...
    /**
     * @brief Extract a list of node-name from a list of vertex_properties_t.
     *
     * @param nodes set of vertex_properties_t
     * @return std::set<std::string> set of node-names
     */
    std::set<std::string> nodes2names(const NODE_SET &nodes) const;

    /**
     * @brief Retrieve the ranges of the incoming vertex_properties_t of node named name.
     *
     * @param name name of a node to check
     * @return VALUERANGES_TYPE rangs of the incoming vertex_properties_t
     */
    ...
    /**
     * @brief Make all vertex_properties_t uniform.
     *
     * @return true, if successful, false otherwise
     */
    bool makeUniform();
    ...
    /**
     * @brief Normalise all vertex_properties_t.
     *
     * @return true, if successful, false otherwise
     */
    bool normalise();
    ...
    /**
     * @brief Canonise all vertex_properties_t,
     *
     * @return true, if successful, false otherwise
     */
    bool canonise();
    ...
    /**
     * @brief Calculate conditionally independent sets of vertex_properties_t (d-separated vertex_properties_t).
     *
     * @param ce conditional event
    ...
    private:
    abstract_base_graph<Node, Dependency> g_;  ///< The underlying graph.
};
