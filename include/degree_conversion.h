...
namespace util
{
    ...
    class TDeg180: public limited_int<T_, static_cast<T_>(-180), static_cast<T_>(180), PolicySetModulo>
    {
        typedef limited_int<T_, static_cast<T_>(-180), static_cast<T_>(180), PolicySetModulo> Base;
    public:
        TDeg180(T_ val = 0)
        : Base(val)
        {
            static_assert(std::is_integral<T_>::value, "Template TDeg180<> requires integral template parameter");
            static_assert(T_(-180) < T_(0), "Cannot instantiate TDeg180 with unsigned type");
...
//        operator T_() {return this->Base::val();}
    };

    template<typename T_>
    class TDeg360 : public limited_int<T_, 0, 360, PolicySetModulo>
    {
        typedef limited_int<T_, 0, 360, PolicySetModulo> Base;
    public:
        TDeg360(T_ val = 0)
        : Base(val)
        {
...
//        operator T_() {return this->Base::val();}
    ...
    {
        typedef limited_int<T_, 0, MICRO_RAD_2PI, PolicySetModulo> Base;
    public:
        TRad2Pi(T_ val = 0)
        : Base(val)
        {
