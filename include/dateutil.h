 ...
 * @return std::time_t
 */
std::time_t pt_to_time_t(val::ptime const &pt);
 ...
 * @return time_t
 */
time_t seconds_from_epoch(std::string const &s);

/**
 *  Scan/parse a string into a date.
 */
val::ptime scanDate(std::string const &s);
 ...
 */
inline bool isTimeOnly(std::string const &s);

/**
 *  Helper to standardise date-strings to always use 2-digits for month/days.
 */
std::string addLeadingZeros(std::string const &str);

/**
 *  Helper to conjugate certain format specifiers' case (\%y<->\%Y,...)
 */
std::string changeCaseOfFormatFlag(std::string const &fmt, char flag);

/**
 *  Add a format to the list of valid formats.
 */
void addDateFormat(std::string const &fmt);
 ...
 */
std::string timestamp(std::string const &fmt = "%Y-%m-%d_%H:%M:%S.%f");

/**
 *  Imbues an output format for dates to an output-stream.
 */
void imbueDateFormat(std::ostream &os, std::string const &fmt);
}; // namespace datescan

/**
 *  Checks whether a time is a valid time.
 */
inline bool valid(val::ptime const &pt)
{
    return !pt.is_infinity() && !pt.is_not_a_date_time() && (pt.date().year() >= 1'900) && (pt.date().year() <= 2'200);
}
}; // namespace util
