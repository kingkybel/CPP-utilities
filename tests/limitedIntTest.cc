...
void testIteratorT()
{
    ...
    try
    {
        for(auto iter = testedTypeThrow::rbegin(); iter != testedTypeThrow::rend(max - 15); iter++)
        {
            // independent of Policy: whilst iterating through valid range *iter should be valid
            CPPUNIT_ASSERT(iter->isValid());
        }
...
void LimitedValuesTest::testDegreeConversion()
{
    ...
    for(int64_t i = -500; i < 500; i++)
        vec180.push_back(i);
    if(vec180.end() != std::find_if(vec180.begin(), vec180.end(), [](Deg180 const &v) {
           return ((v.val() > Deg180::max()) || (v.val() < Deg180::min()));
       }))
    {
    ...
    for(int64_t i = -500; i < 500; i++)
        set360.insert(i);
    if(set360.end() != std::find_if(set360.begin(), set360.end(), [](Deg360 const &v) {
           return ((v.val() > Deg360::max()) || (v.val() < Deg360::min()));
       }))
    {
